...
watch(search, debounce((value) => {
    router.get(route('cp.schedules.templates'), { search: value }, {
        preserveState: true,
        replace: true
    });
}, 300));
...
<template>
    <Head title="Modèles de Planning — CP" />
    <CpLayout>
        <template #header>
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <Link :href="route('cp.schedules.assign-sup')" class="px-4 py-2 bg-white border border-pearl-200 rounded-lg text-xs font-bold text-charcoal-600 hover:bg-pearl-50 transition-all flex items-center gap-2 shadow-sm">
                    ...
                    <Link :href="route('cp.schedules.templates.create')" class="px-4 py-2 bg-gold-gradient rounded-lg text-xs font-bold text-white hover:opacity-90 transition-all flex items-center gap-2 shadow-gold">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            <div v-for="model in modelsData" :key="model.id" class="group bg-white rounded-xl border border-pearl-200 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
                ...
                <div class="flex border-t border-pearl-100">
                    <Link :href="route('cp.schedules.templates.edit', model.id)" class="flex-1 py-3 text-[10px] font-bold text-charcoal-500 uppercase tracking-widest hover:bg-pearl-50 hover:text-gold-600 transition-colors flex items-center justify-center gap-2 border-r border-pearl-100">
            ...
            <Link v-if="props.models.current_page === 1" :href="route('cp.schedules.templates.create')" class="group flex flex-col items-center justify-center p-8 bg-white/50 border-2 border-dashed border-pearl-200 rounded-xl hover:border-gold-300 hover:bg-white transition-all min-h-[300px]">
