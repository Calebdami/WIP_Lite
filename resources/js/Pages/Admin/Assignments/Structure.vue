    ...
    <AdminLayout>
            ...
            <div v-for="(struct, campaignId) in campaignsStructure" :key="campaignId" class="bg-white rounded-2xl border border-pearl-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                ...
                <div class="p-6">
                    ...
                    <div v-else class="space-y-8">
                        <!-- Orphan CP Placeholder (If there are SUPs without an active CP) -->
                        <div v-if="struct.orphanSups.length > 0" class="relative pl-6 border-l-2 border-rose-200 bg-rose-50/30 rounded-2xl p-6 border-dashed">
                            <div class="absolute top-0 left-0 w-6 h-0.5 bg-rose-200 -ml-[2px] mt-4"></div>
                            <div class="flex items-center gap-5 mb-8">
                                <div class="w-14 h-14 rounded-2xl bg-white border-2 border-rose-200 border-dashed flex items-center justify-center text-rose-300 shadow-sm">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-[10px] text-rose-400 font-bold uppercase tracking-widest mt-0.5">Voudriez-vous en ajouter ? (Chef de Plateau)</p>
                                </div>
                                <button 
                                    @click="activeTab = 'available'"
                            ...
                            <!-- Orphan SUPs display -->
                            <div class="space-y-6">
                                <div v-for="sup in struct.orphanSups" :key="sup.id" class="relative pl-6 border-l-2 border-rose-100 last:border-l-0">
                                    <div class="absolute top-0 left-0 w-6 h-0.5 bg-rose-100 -ml-[2px] mt-4"></div>
                                    <!-- Rest of SUP display same as normal -->
                                    <div class="group bg-white p-4 rounded-xl border border-pearl-200 shadow-sm hover:border-gold-300 hover:shadow-md transition-all">
                                        <div class="flex items-center justify-between">
                                            <div class="flex items-center gap-3">
                                                <div class="w-8 h-8 rounded-lg bg-pearl-50 flex items-center justify-center text-charcoal-700 font-bold text-[10px] border border-pearl-100">
                                                    {{ sup.employee?.first_name.charAt(0) }}{{ sup.employee?.last_name.charAt(0) }}
                                                </div>
                                                <div>
                                                    <div class="text-xs font-bold text-charcoal-700">{{ sup.employee?.first_name }} {{ sup.employee?.last_name }}</div>
                                                    <div class="text-[9px] text-charcoal-400 font-bold uppercase tracking-tighter">Superviseur</div>
                                                </div>
                                            </div>
                                            <button @click="openReleaseModal(sup)" class="p-1.5 text-pearl-300 hover:text-rose-500 transition-colors">
                                        ...
                                        <div v-if="sup.tcs.length > 0" class="mt-4 space-y-3">
                                            <div v-for="tc in sup.tcs" :key="tc.id" class="relative pl-6 border-l-2 border-pearl-100 last:border-l-0">
                                                <div class="absolute top-0 left-0 w-4 h-0.5 bg-pearl-100 -ml-[2px] mt-3"></div>
                                                <div class="flex items-center justify-between bg-pearl-50/50 px-3 py-2 rounded-lg border border-pearl-100 group-hover:border-pearl-200">
                                                    <div class="flex items-center gap-2">
                                                        <div class="text-[10px] font-bold text-charcoal-600">{{ tc.employee?.first_name }} {{ tc.employee?.last_name }}</div>
                                                        <div class="text-[8px] px-1.5 py-0.5 bg-white border border-pearl-200 rounded text-charcoal-400 font-bold uppercase tracking-tighter">TC</div>
                                                    </div>
                                                    <button @click="openReleaseModal(tc)" class="text-pearl-300 hover:text-rose-500 transition-colors">
                        ...
                        <!-- Orphan TC Placeholder at Campaign Level (Missing SUP) -->
                        <div v-if="struct.orphanTcs.length > 0" class="relative pl-6 border-l-2 border-amber-100 bg-amber-50/20 rounded-xl p-4 border-dashed mt-4">
                            <div class="absolute top-0 left-0 w-6 h-0.5 bg-amber-100 -ml-[2px] mt-3"></div>
                            <div class="flex items-center gap-3 mb-4">
                                <div class="w-8 h-8 rounded-lg bg-white border border-amber-200 border-dashed flex items-center justify-center text-amber-400">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                </div>
                                <div>
                                    <h4 class="text-[10px] font-black text-amber-600 uppercase tracking-tight">Aucun Superviseur actuellement</h4>
                                    <p class="text-[8px] text-amber-400 font-bold uppercase tracking-widest">Des TCs attendent un chef</p>
                                </div>
                                <button @click="activeTab = 'available'" class="ml-auto p-1 bg-amber-500 text-white rounded-md hover:bg-amber-600 transition-all shadow-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                                <div v-for="tc in struct.orphanTcs" :key="tc.id" class="flex items-center justify-between bg-white px-3 py-2 rounded-lg border border-amber-100 shadow-sm">
                                    <div class="flex items-center gap-2">
                                        <div class="text-[10px] font-bold text-charcoal-700">{{ tc.employee?.first_name }} {{ tc.employee?.last_name }}</div>
                                        <div class="text-[8px] px-1.5 py-0.5 bg-pearl-50 border border-pearl-100 rounded text-charcoal-400 font-bold uppercase tracking-tighter">TC</div>
                                    </div>
                                    <button @click="openReleaseModal(tc)" class="text-pearl-300 hover:text-rose-500 transition-colors">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                    </button>
        ...
        <div v-if="activeTab === 'available'">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div v-for="emp in availableEmployees.data" :key="emp.id" class="bg-white rounded-2xl border border-pearl-200 p-5 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
                    <div class="flex items-center gap-3 mb-4">
            ...
            <div v-if="availableEmployees.links.length > 3" class="mt-8 flex justify-center">
                <div class="flex gap-1 bg-white p-1 rounded-xl border border-pearl-200 shadow-sm">
                    <Link 
                        v-for="link in availableEmployees.links" 
                        :key="link.label"
                        :href="link.url || '#'"
                        v-html="link.label"
                        class="px-4 py-2 rounded-lg text-xs font-bold transition-all"
            ...
            <div v-if="availableEmployees.data.length === 0" class="text-center py-20 bg-white rounded-2xl border border-pearl-200">
                ...
                <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden border border-pearl-100">
                    ...
                    <div class="p-8 space-y-8">
                        <p class="text-charcoal-600 text-sm font-medium leading-relaxed">
                            Voulez-vous vraiment libérer <strong>{{ selectedAssignment?.employee?.first_name }} {{ selectedAssignment?.employee?.last_name }}</strong> ? Cette action mettra fin à son affectation actuelle.
                        </p>
