...
<template>
    <Head title="Nouveau Modèle de Planning — Admin" />
    <AdminLayout>
        <template #header>
            ...
            <div class="space-y-6">
                        ...
                        <div class="pt-4">
                            <div class="h-2 w-full bg-pearl-100 rounded-full overflow-hidden flex">
                                <div 
                                    v-for="day in days" 
                                    :key="day.key"
                                    :style="{ width: (parseFloat(form[day.key]) / totalHours * 100) + '%' }"
                ...
                <div class="bg-white rounded-xl border border-pearl-200 p-6 shadow-sm">
                    ...
                    <ul class="space-y-3">
                        ...
                        <li class="flex items-start gap-3">
                            <div class="w-1.5 h-1.5 rounded-full bg-gold-400 mt-1.5 flex-shrink-0"></div>
                            <p class="text-xs text-charcoal-500 leading-relaxed">Un modèle peut être affecté à <strong>plusieurs employés</strong>.</p>
                        </li>
                    </ul>
                </div>
            </div>
