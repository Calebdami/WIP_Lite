...
const submitReport = () => {
    form.post(route('tc.reports.store'), {
        onSuccess: () => {
...
<template>
    <Head title="Mes Rapports Journaliers" />

    <TcLayout>
        <template #header>
            <div class="flex justify-between items-center gap-2">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Mes Rapports Journaliers</h2>
                <div class="flex gap-2">
                    <a :href="route('tc.reports.export')" class="p-button p-component p-button-secondary">
                    ...
                    <template #content>
                        <DataTable :value="reports.data" paginator :rows="10" tableStyle="min-width: 50rem">
                            ...
                            <Column field="status" header="Statut">
                                <template #body="slotProps">
                                    <span class="px-2 py-1 rounded text-xs" :class="slotProps.data.status === 'submitted' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'">
