    ...
    <SupLayout>
        ...
        <Dialog v-model:visible="displayEditDialog" :header="`Saisie des Heures`" :style="{ width: '1000px' }" modal maximizable>
            ...
            <DataTable :value="timesheetEntries" responsiveLayout="scroll" class="p-datatable-sm" editMode="cell">
                ...
                <Column header="Arrivée" headerClass="w-32">
                    <template #body="{ data }">
                        <InputText v-model="data.check_in" placeholder="09:00" class="w-full text-center font-mono" />
                    </template>
                </Column>
                <Column header="Départ" headerClass="w-32">
                    <template #body="{ data }">
                        <InputText v-model="data.check_out" placeholder="17:00" class="w-full text-center font-mono" />
                    </template>
                </Column>
                <Column header="Pause (min)" headerClass="w-28">
                    <template #body="{ data }">
                        <InputNumber v-model="data.break_duration" class="w-full" :min="0" :max="120" />
                    </template>
                </Column>
                <Column header="Absence / Motif">
                    <template #body="{ data }">
                        <InputText v-model="data.absence_type" placeholder="Type d'absence..." class="w-full p-inputtext-sm" />
                    </template>
                </Column>
            ...
            <template #footer>
                <div class="flex justify-between items-center w-full">
                    <p class="text-xs text-charcoal-400 italic">Toutes les modifications sont enregistrées localement avant validation.</p>
                    <div class="flex gap-2">
                        <Button label="Annuler" text severity="secondary" @click="displayEditDialog = false" />
                        <Button label="Enregistrer les heures" icon="pi pi-save" severity="primary" @click="saveEntries" :loading="savingEntries" />
                    </div>
                </div>
            </template>
        </Dialog>
